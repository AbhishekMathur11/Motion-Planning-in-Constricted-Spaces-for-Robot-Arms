...
def CheckPointOverlap(pointsA, pointsB, axis):	
    ...
    proj_points_A = np.array([np.dot(pointsA[i], axis) for i in range(9)])
    proj_points_B = np.array([np.dot(pointsB[i], axis) for i in range(9)])

    min_A = np.min(proj_points_A)
    max_A = np.max(proj_points_A)
    min_B = np.min(proj_points_B)   
    max_B = np.max(proj_points_B)
