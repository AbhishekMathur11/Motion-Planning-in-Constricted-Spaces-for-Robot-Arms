...
def PRMQuery():
    ...
    # open road map
    f = open("/home/abhishek/MRSD/Robot Autonomy/MyPRM.p", 'rb')
    prmVertices = pickle.load(f)
    prmEdges = pickle.load(f)
    pointsObs = pickle.load(f)
    axesObs = pickle.load(f)
    f.close
    print(len(prmVertices), " vertices loaded")
    print(len(prmEdges), " edges loaded")
    print(len(pointsObs), " pointsObs loaded")
    print(len(axesObs), " axesObs loaded")
    ...
    print("Number of nodes in the plan: ", len(activenodes))
    plan = [activenodes[-1]]
